...
Cosine of each element (input is in radians).
...
.. plotly::
    ...
    from screamer import Cos

    np.random.seed(0)
    data = np.random.normal(size=80) * 1.5
    transformed = Cos()(data)
    ...
    fig.update_layout(
        title="Cos transformation",
        margin=dict(l=20, r=20, t=60, b=20),
        legend=dict(orientation="h", yanchor="bottom", y=1.02,
                    xanchor="right", x=1),
    )
    fig.update_yaxes(title_text="input", row=1, col=1)
    fig.update_yaxes(title_text="Cos(input)", row=2, col=1)
    fig.show()
```
