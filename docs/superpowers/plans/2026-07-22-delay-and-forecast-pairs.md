...
**Placeholder scan:** No TBD/TODO. Task 2 Step 1 shows a scratch live-test draft immediately replaced by the real `.live()` version in the same step; the real version is the one to implement. All C++ is exact (from the traced Resample recipe). Duration-mode column/dtype adjustments are called out as "match the actual CombineLatest output" with the invariant stated, which is guidance for a known-variable API surface, not a placeholder for missing logic.
