...
deriving FunctorBase) -- and records its constructor signature. Ops named in
``DYNAMIC_REDUCER_CLASS_NAMES`` additionally carry ``"dynamic_reducer": true``,
which tells the binding and TypeScript generators to emit the variable-width
reduction entry points on top of the ordinary fixed-width ones.
...
# Dynamic-width reducers keep the fixed n_in/n_out event contract and add a
# call_assets(row, groups) path that folds a variable number of groups into one
# event. The group count is a data-shape dimension, so it travels as a call
# argument instead of living in the op's type. These ops are marked in the
# manifest so the downstream generators emit the extra reducer entry points
# alongside the ordinary ones.
DYNAMIC_REDUCER_CLASS_NAMES = {"PortfolioReport"}
...
def parse_file(path: Path) -> list[dict]:
    ...
    for m in _CLASS_RE.finditer(text):
        ...
        # Only reducers carry the flag, so introducing the category leaves
        # every ordinary op's manifest entry byte-identical.
        if name in DYNAMIC_REDUCER_CLASS_NAMES:
            entry["dynamic_reducer"] = True
        ops.append(entry)
...
def validate(ops: list[dict]) -> None:
    ...
    for name in sorted(DYNAMIC_REDUCER_CLASS_NAMES):
        assert name in by_name, f"dynamic-width reducer {name} missing from manifest"
        assert by_name[name].get("dynamic_reducer") is True, (
            f"{name}.dynamic_reducer: expected True, got "
            f"{by_name[name].get('dynamic_reducer')!r}"
        )
