...
def test_resample_by_key_batch_stream_oracle(agg):
    ...
    dag = Pipeline(inputs=[x], outputs=[Resample(freq=10, agg=agg)(x)])
...
def test_resample_by_key_right_label_and_origin():
    ...
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[Resample(freq=5, origin=2, agg="sum", label="right")(x)])
    (bv, bk), (sv, sk) = _stream_1d(dag, (vals, keys))
...
def test_resample_ohlc_4col_in_graph():
    ...
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[Resample(freq=10, agg="ohlc")(x)])
    bv, bk = dag((vals, keys))
    sv, sk = _lazy_batch(dag, (vals, keys))
...
def test_resample_trailing_bucket_flush_batch_equals_stream():
    ...
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[Resample(freq=10, agg="last")(x)])
    bv, bk = dag((vals, keys))
    sv, sk = _lazy_batch(dag, (vals, keys))
...
def test_resample_flush_through_combine_latest():
    ...
    a, b = Input("a"), Input("b")
    dag = Pipeline(inputs=[a, b], outputs=[Resample(freq=10, agg="last")(Sub()(CombineLatest()(a, b)))])
...
def test_resample_feeds_functor():
    ...
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[RollingMean(2)(Resample(freq=10, agg="mean")(x))])
    bv, bk = dag((vals, keys))
    sv, sk = _lazy_batch(dag, (vals, keys))
...
def test_resample_by_count_batch_stream_oracle(agg):
    ...
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[Resample(count=2, agg=agg)(x)])
    bv, bk = dag((vals, keys))
    sv, sk = _lazy_batch(dag, (vals, keys))
...
def test_resample_by_count_right_label_in_graph():
    ...
    vals = np.array([1.0, 2.0, 3.0, 4.0])
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[Resample(count=2, agg="last", label="right")(x)])
    bv, bk = dag((vals, keys))
    sv, sk = _lazy_batch(dag, (vals, keys))
...
def test_resample_by_count_ohlc_in_graph():
    ...
    vals = np.array([4.0, 2.0, 8.0, 6.0, 7.0])
    x = Input("x")
    dag = Pipeline(inputs=[x], outputs=[Resample(count=2, agg="ohlc")(x)])
    bv, bk = dag((vals, keys))
    sv, sk = _lazy_batch(dag, (vals, keys))
